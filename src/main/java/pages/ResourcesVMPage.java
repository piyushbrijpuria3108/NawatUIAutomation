...
//	xpaths for Virtual Machine
	...
	@FindBy(xpath="//div[contains(text(), 'Virtual machine')]")
	private WebElement ValidateVMModuleName;
	
	@FindBy(xpath="//span[contains(text(), 'test-access')]")
	WebElement ValidateVMSearchData;
	
	@FindBy(xpath="//div[contains(@class, 'chip-ellip')]")
	WebElement ValidateVMFilterData;

	
//	Method for VMs
	public void matchVMPageColumns() {
		...
		for (WebElement component : tableVMPage) {
		...
		// Comparing both lists and failing test if they don't match
		 Assert.assertEquals(actualList, expectedList, "❌ Virtual Machine columns do no match.");
	}
...
//		searchIcon.sendKeys("test");
		...
		String searchValue = ExcelDataReader.readSingleRowData(excelFilePath, 0, 0, 2); // Sheet 0, Column 0, Row 1
		...
		String Text = searchIcon.getText();
		Thread.sleep(3000);
		System.out.println("The name of the searched VM is " + Text);
		closeSearchIcon.click();
	}
	
	public void clickFilterIconOnVMPage() throws InterruptedException {
		
		waitForElementToBeClickable(clickFilterIcon);
		...
		Thread.sleep(2000);
		FilterEnterName.sendKeys("test-access");
...
//	Assertion Methods
	...
	public String ValidateVMModuleName() {
		
		waitForElementToBeVisible(ValidateVMModuleName);
		return ValidateVMModuleName.getText();
	}
	
    public String ValidateVMSearchData() {
		
		waitForElementToBeVisible(ValidateVMSearchData);
		return ValidateVMSearchData.getText();
	}
    
    public String ValidateVMFilterData() {
		
		waitForElementToBeVisible(ValidateVMFilterData);
		return ValidateVMFilterData.getText();
	}
}